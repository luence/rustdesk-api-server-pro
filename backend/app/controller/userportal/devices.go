...
func (c *DevicesController) HandleMyDevices() mvc.Result {
	...
	if user.IsAdmin {
		return c.listAllDevices(currentPage, pageSize)
	}
	return c.listMyDevices(user.Id, currentPage, pageSize)
}

func (c *DevicesController) listAllDevices(currentPage, pageSize int) mvc.Result {
	pagination := db.NewPagination(currentPage, pageSize)
	deviceList := make([]model.Device, 0)

	query := func() *xorm.Session {
		return c.Db.Table(&model.Device{}).Desc("updated_at")
	}
...
func (c *DevicesController) listMyDevices(userID int, currentPage, pageSize int) mvc.Result {
	type idRow struct {
