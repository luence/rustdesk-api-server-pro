...
默认不自动创建账号。首次登录或删除绑定后，Web 页面必须先拦截并要求用户明确选择：输入目标本地账户及其密码执行“绑定并登录”，或在启用 `autoCreateUser` 时点击“创建普通用户并登录”。回调阶段不静默建号；只有已存在有效绑定记录时才直接登录。Provider 不预设本地账户角色，角色由目标账户决定，禁止自动创建管理员。
