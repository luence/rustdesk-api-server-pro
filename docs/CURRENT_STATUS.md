...
### 客户端 OAuth/WebAuth 结果传递
- **根因**：管理员完成网页登录后，首次 `/api/oidc/auth-query` 会消费 ticket，但兑换逻辑错误地只接受 `is_admin=0` 的用户，首次返回账号错误，后续持续返回 `ERR-2008`。
- **修复**：允许管理员作为客户端登录用户，客户端 token 仍强制 `is_admin=false`；成功结果保持幂等缓存。已添加 WebAuth 管理员回归测试。
- **下一步**：部署后分别验收 WebAuth 和第三方 Provider 的官方客户端轮询流程。
