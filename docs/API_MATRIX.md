...
| 通讯录管理 | `/admin/ab/*` | 完整 | 联系人/标签支持当前账户全量列表和地址簿名称筛选；管理员新增联系人、标签、地址簿时显式选择用户并按用户过滤地址簿，`created_by_admin` 在后端阻止普通用户删除管理员代建地址簿；新增型页面支持 CSV 导入导出 |
