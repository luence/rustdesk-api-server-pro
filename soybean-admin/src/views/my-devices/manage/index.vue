...
const filteredData = computed(() =>
  data.value.filter(row =>
    Object.entries(filters).every(
      ([key, value]) =>
        !value ||
        String(key === 'name' ? localizeAddressBookName((row as any)[key]) : (row as any)[key] || '')
          .toLowerCase()
...
const columns = [
  ...
  {
    key: 'name',
    title: filterTitle($t('dataMap.ab.name'), 'name'),
    align: 'center' as const,
    render: (row: Api.AddressBook.AddressBook) => localizeAddressBookName(row.name)
  },
  ...
  { key: 'max_peer', title: $t('dataMap.ab.max_peer'), align: 'center' as const },
  {
    ...
    render: (row: Api.AddressBook.AddressBook) => (
      <NTag type={row.shared ? 'success' : 'default'}>
        {row.shared ? $t('common.yesOrNo.yes') : $t('dataMap.ab.personal')}
      </NTag>
    )
  },
  {
    ...
    render: (row: Api.AddressBook.AddressBook) =>
      row.shared ? (
        <NSpace justify="center">
          <NButton size="small" quaternary type="primary" onClick={() => openEdit(row)}>
            {$t('common.edit')}
          </NButton>
          {(!row.created_by_admin || isAdmin.value) && (
...
function exportData() {
  downloadCsv('address-books.csv', filteredData.value as any[], [
    'user_id',
    'owner',
    'guid',
    'name',
...
async function importData(event: Event) {
  ...
  for (const row of rows) {
    ...
    const result = await fetchAbSharedAdd({
      user_id: Number(row.user_id) || 0,
      name: row.name,
      note: row.note,
      rule: Number(row.rule) || 1,
      max_peer: Number(row.max_peer) || 0
    } as any);
...
<template>
  <div class="min-h-500px flex-col-stretch gap-16px overflow-hidden lt-sm:overflow-auto">
    <NCard :title="$t('route.my-devices_manage')" :bordered="false" size="small" class="sm:flex-1-hidden card-wrapper">
      <template #header-extra>
        <NSpace>
          ...
          <NButton type="primary" size="small" @click="openAdd">{{ $t('common.add') }}</NButton>
        </NSpace>
      </template>
      <NDataTable
        :columns="columns"
    ...
    <NModal
      ...
      class="max-w-560px lt-sm:max-w-full lt-sm:w-full"
    >
      <NForm :label-placement="appStore.isMobile ? 'top' : 'left'" :label-width="appStore.isMobile ? undefined : 120">
        <NFormItem v-if="isAdmin && !editing" :label="$t('dataMap.ab.owner')">
          <NSelect v-model:value="form.user_id" :options="userOptions" filterable />
        </NFormItem>
        ...
        <NFormItem :label="$t('dataMap.ab.rule')">
          <NSelect
            v-model:value="form.rule"
            :options="[
              { label: $t('dataMap.ab.read'), value: 1 },
              { label: $t('dataMap.ab.readWrite'), value: 2 },
              { label: $t('dataMap.ab.fullControl'), value: 3 }
            ]"
          />
        </NFormItem>
        <NFormItem :label="$t('dataMap.ab.max_peer')">
          <NInputNumber v-model:value="form.max_peer" :min="0" />
        </NFormItem>
      </NForm>
      <template #footer>
        <NSpace justify="end">
