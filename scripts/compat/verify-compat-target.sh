...
repository_server_version="$(tr -d '[:space:]' < "$VERSION_FILE")"
service_sysinfo="rustdesk-api-server-pro-compat-client-${service_client_version}-server-${repository_server_version}-latest"
...
check_equal server.compat_server_version "$manifest_server_version" "$repository_server_version"
...
done < <(jq -r '.public_probe_endpoints[]' "$CURRENT_FILE")

echo "Compatibility target manifest matches Go constants and VERSION=${repository_server_version}"
