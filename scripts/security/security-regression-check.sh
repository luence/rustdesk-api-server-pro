...
# Scheduler startup must reuse the app DB engine and return errors instead of panicking.
...
grep -q 'dbEngine, err := db.NewEngine(cfg.Db)' backend/app/main.go || fail "server startup must create the DB engine once"
...
grep -q 'OauthUserinfoSubjectMismatch' backend/internal/service/oauth_provider_service.go || grep -q 'ERR-2013.*OauthUserinfoSubjectMismatch' backend/internal/errcode/errcode.go || fail "OAuth provider userinfo/id_token subject consistency check missing"
...
grep -q 'OidcUnsupportedIdTokenAlg\|ERR3026' backend/internal/service/oidc_jwks_verify.go || grep -q 'ERR-3026' backend/internal/errcode/errcode.go || fail "OIDC alg allowlist missing"
grep -q 'validateIDTokenClaims' backend/internal/service/oidc_auth_service.go || fail "OIDC ID token claim validation missing"
grep -q 'OidcIdTokenIssuerInvalid\|ERR3014' backend/internal/service/oidc_auth_service.go || grep -q 'ERR-3014' backend/internal/errcode/errcode.go || fail "OIDC issuer validation missing"
grep -q 'OidcIdTokenAudienceInvalid\|ERR3015' backend/internal/service/oidc_auth_service.go || grep -q 'ERR-3015' backend/internal/errcode/errcode.go || fail "OIDC audience validation missing"
grep -q 'OidcIdTokenExpired\|ERR3016' backend/internal/service/oidc_auth_service.go || grep -q 'ERR-3016' backend/internal/errcode/errcode.go || fail "OIDC expiry validation missing"
grep -q 'OidcIdTokenIssuedAtInvalid\|ERR3017' backend/internal/service/oidc_auth_service.go || grep -q 'ERR-3017' backend/internal/errcode/errcode.go || fail "OIDC issued-at validation missing"
